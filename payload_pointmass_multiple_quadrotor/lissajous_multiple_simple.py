...
ACCELS_PLOT_PATH = Path(__file__).with_name(
    "trajectory_accelerations_lissajous_payload.png"
)
...
def _progress_profile(
    ...
    if tc < tr:
        ...
        sddot = (
            7.0 * a7 * tc**6 + 6.0 * a6 * tc**5 + 5.0 * a5 * tc**4 + 4.0 * a4 * tc**3
        )
        sdddot = (
            42.0 * a7 * tc**5
            + 30.0 * a6 * tc**4
        ...
        sddddot = (
            210.0 * a7 * tc**4 + 120.0 * a6 * tc**3 + 60.0 * a5 * tc**2 + 24.0 * a4 * tc
        )
        ...
        sddot = (
            -7.0 * a7 * te**6 - 6.0 * a6 * te**5 - 5.0 * a5 * te**4 - 4.0 * a4 * te**3
        )
        sdddot = (
            42.0 * a7 * te**5
            + 30.0 * a6 * te**4
...
def _lissajous_payload_state(
    ...
    a[0] = x_amp * (
        x_coeff2 * np.cos(x_coeff * s) * sdot * sdot
        + x_coeff * np.sin(x_coeff * s) * sddot
    )
    a[1] = y_amp * (
        -y_coeff2 * np.sin(y_coeff * s) * sdot * sdot
        + y_coeff * np.cos(y_coeff * s) * sddot
    )
    a[2] = z_amp * (
        -z_coeff2 * np.sin(z_coeff * s) * sdot * sdot
        + z_coeff * np.cos(z_coeff * s) * sddot
    )

    j[0] = x_amp * (
        -x_coeff3 * np.sin(x_coeff * s) * sdot**3
        + 3.0 * x_coeff2 * np.cos(x_coeff * s) * sdot * sddot
        + x_coeff * np.sin(x_coeff * s) * sdddot
    )
    j[1] = y_amp * (
        -y_coeff3 * np.cos(y_coeff * s) * sdot**3
        - 3.0 * y_coeff2 * np.sin(y_coeff * s) * sdot * sddot
        + y_coeff * np.cos(y_coeff * s) * sdddot
    )
    j[2] = z_amp * (
        -z_coeff3 * np.cos(z_coeff * s) * sdot**3
        - 3.0 * z_coeff2 * np.sin(z_coeff * s) * sdot * sddot
        + z_coeff * np.cos(z_coeff * s) * sdddot
    )
...
def plan_three_quad_lissajous_payload(
    ...
    for k, t in enumerate(times):
        s, sdot, sddot, sdddot, sddddot, _ = _progress_profile(
            t, period, num_cycles, ramp_time
        )
        ...
        for i in range(3):
            ...
            cable = reconstruct_cable(
                lambdas[i, k], lambdas_dot[i, k], lambdas_ddot[i, k]
            )
